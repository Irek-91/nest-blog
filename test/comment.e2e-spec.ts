...
describe('tests for comments', () => {
  ...
  describe('действия с коментариями', () => {
    it('обновление коментария ', async () => {
      ...
      const resultCreated = await request(httpServer).get(
        `/comments/${commentId}`,
      );
      ...
      const resultUpdate = await request(httpServer).get(
        `/comments/${commentId}`,
      );
    ...
    it('лайк коментария', async () => {
      ...
      const resultOne = await request(httpServer)
      ...
      const resultTwo = await request(httpServer)
        .put(`/comments/${comment.id}/like-status`)
        .set(headersJWTTwo)
        .send(likeStatusDataTwo)
        .expect(204);

      const resultUpdateLikeForUserOne = await request(httpServer)
      ...
      const resultUpdateLikeForUserTwo = await request(httpServer)
        .get(`/comments/${comment.id}`)
        .set(headersJWTTwo);
