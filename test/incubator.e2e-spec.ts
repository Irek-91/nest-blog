...
import { createBlogByBlogger } from './helpers/blogs-tests-helpers';
...
describe('tests for blogger', () => {
  ...
  describe('create blogger tests', () => {
    it('Создаем пользователей', async () => {
      ...
      const userFree = await createUser(
        'admin',
        'qwerty',
        userFreeModel,
        httpServer,
    ...
    it(`Telegram api integration  blog subscriptions  DELETE -> "blogs/:blogId/subscription"
      ...
      const blogOne = await createBlogByBlogger(
        userOne.user.id,
        model,
        httpServer,
      );
      ...
      const post = await request(httpServer)
        .post(`/blogger/blogs/${blogOne.body.id}/posts`)
        .send(postData)
        .set(userOne.headers);
      expect(post.status).toBe(201);
      expect.setState({ postByUserOneBlog1: post.body });

      const subscription = await request(httpServer)
        .post(`/blogs/${blogOne.body.id}/subscription`)
        .set(userTwo.headers);
      expect(subscription.status).toBe(204);

      const unsubscription = await request(httpServer)
        .delete(`/blogs/${blogOne.body.id}/subscription`)
        .set(userTwo.headers);
      expect(unsubscription.status).toBe(204);

      const GetBlog1ByUser2 = await request(httpServer)
        .get(`/blogs/${blogOne.body.id}`)
        .set(userTwo.headers);
      ...
      const GetBlog1ByUser3 = await request(httpServer)
        .get(`/blogs/${blogOne.body.id}`)
        .set(userFree.headers);
      ...
      const GetBlog2ByUnauthorizedUser = await request(httpServer).get(
        `/blogs/${blogOne.body.id}`,
      );
    ...
    it(`Telegram api integration  blog subscriptions  POST -> "blogs/:blogId/subscription"
      ...
      const blog2 = await createBlogByBlogger(
        userOne.user.id,
        model,
        httpServer,
      );
      ...
      const subscription = await request(httpServer)
        .post(`/blogs/${blog2.body.id}/subscription`)
        .set(userTwo.headers);
      expect(subscription.status).toBe(204);

      const tryToSubscribe = await request(httpServer)
        .post(`/blogs/${blog2.body.id}/subscription`)
        .set(userTwo.headers);
      expect(tryToSubscribe.status).toBe(204);

      const GetBlog3ByUser2 = await request(httpServer)
        .get(`/blogs/${blog2.body.id}`)
        .set(userTwo.headers);
    ...
    it(`Telegram api integration  blog subscriptions  DELETE -> "blogs/:blogId/subscription": 
      ...
      const blog3 = await createBlogByBlogger(
        userOne.user.id,
        model,
        httpServer,
      );
      ...
      const subscription = await request(httpServer)
        .post(`/blogs/${blog3.body.id}/subscription`)
        .set(userTwo.headers);
      expect(subscription.status).toBe(204);
      const subscription2 = await request(httpServer)
        .post(`/blogs/${blog3.body.id}/subscription`)
        .set(userFree.headers);
      expect(subscription2.status).toBe(204);
      const unsubscription = await request(httpServer)
        .delete(`/blogs/${blog3.body.id}/subscription`)
        .set(userTwo.headers);
      expect(unsubscription.status).toBe(204);
      const unsubscription2 = await request(httpServer)
        .delete(`/blogs/${blog3.body.id}/subscription`)
        .set(userTwo.headers);
      expect(unsubscription.status).toBe(204);

      const GetBlog4ByUser2 = await request(httpServer)
        .get(`/blogs/${blog3.body.id}`)
        .set(userTwo.headers);
    ...
    it(`Telegram api integration  blog subscriptions  DELETE -> "blogs/:blogId/subscription": 
      ...
      const blog4 = await createBlogByBlogger(
        userOne.user.id,
        model,
        httpServer,
      );
      ...
      const subscriptionUserTwo1 = await request(httpServer)
        .post(`/blogs/${blog1.id}/subscription`)
        .set(userTwo.headers);
      expect(subscriptionUserTwo1.status).toBe(204);
      const subscriptionUserTwo2 = await request(httpServer)
        .post(`/blogs/${blog2.id}/subscription`)
        .set(userTwo.headers);
      expect(subscriptionUserTwo2.status).toBe(204);
      const subscriptionUserTwo3 = await request(httpServer)
        .post(`/blogs/${blog4.body.id}/subscription`)
        .set(userTwo.headers);
      expect(subscriptionUserTwo3.status).toBe(204);

      const subscriptionUserFree1 = await request(httpServer)
        .post(`/blogs/${blog1.id}/subscription`)
        .set(userFree.headers);
      expect(subscriptionUserFree1.status).toBe(204);
      const subscriptionUserFree2 = await request(httpServer)
        .post(`/blogs/${blog2.id}/subscription`)
        .set(userFree.headers);
      expect(subscriptionUserFree2.status).toBe(204);
      const subscriptionUserFree3 = await request(httpServer)
        .post(`/blogs/${blog4.body.id}/subscription`)
        .set(userFree.headers);
      expect(subscriptionUserFree3.status).toBe(204);

      const unsubscriptionUserTwo = await request(httpServer)
        .delete(`/blogs/${blog4.body.id}/subscription`)
        .set(userTwo.headers);
      expect(unsubscriptionUserTwo.status).toBe(204);

      const GetBlogsByUser2 = await request(httpServer)
        .get(`/blogs`)
        .set(userTwo.headers);
