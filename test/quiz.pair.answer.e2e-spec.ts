...
describe('tests for questions', () => {
    ...
    describe('Отправить ответ на следующий неотвеченный вопрос в активной паре - answer', () => {
        ...
        it('Возвращает статус 403, Если текущий пользователь не входит в активную пару или же еще пара не активна', async () => {
            ...
            const resultCreatedFiveQusetions = await createFiveQuestions('admin', 'qwerty', httpServer)
            ...
            const AccessTokenTwo = jwt.sign({ userId: userTwo.user.id }, settings.JWT_SECRET, { expiresIn: 100 })
            ...
            const userThree = await createUser('admin', 'qwerty', modelThree, httpServer)
            const AccessTokenThree = jwt.sign({ userId: userThree.user.id }, settings.JWT_SECRET, { expiresIn: 100 })
        ...
        it('Возвращает статус 201, после ответа первого пользователя', async () => {
            ...
            const resultAnswer1 = await sendAnswerOneByUser(userOne, inputData, httpServer)
            const resultAnswer2 = await sendAnswerOneByUser(userOne, inputData, httpServer)
            const resultAnswer3 = await sendAnswerOneByUser(userOne, inputData, httpServer)
            const resultAnswer4 = await sendAnswerOneByUser(userOne, inputData, httpServer)
            const resultAnswer5 = await sendAnswerOneByUser(userOne, inputData, httpServer)
        })

        it('Возвращает статус 201, после четырех правильных ответов второго пользователя', async () => {
            const { userTwo } = expect.getState()
            const AccessTokenOne = jwt.sign({ userId: userTwo.id }, settings.JWT_SECRET, { expiresIn: 100 })
            ...
            const resultAnswer1 = await sendAnswerOneByUser(userTwo, inputData, httpServer)
            const resultAnswer2 = await sendAnswerOneByUser(userTwo, inputData, httpServer)
            const resultAnswer3 = await sendAnswerOneByUser(userTwo, inputData, httpServer)
            const resultAnswer4 = await sendAnswerOneByUser(userTwo, inputData, httpServer)
        ...
        it('Возвращает статус 403, после 6 ответа первого пользователя', async () => {
            ...
            const AccessTokenOne = jwt.sign({ userId: userOne.id }, settings.JWT_SECRET, { expiresIn: 100 })
            const headersJWTOne = { Authorization: `Bearer ${AccessTokenOne}` }
