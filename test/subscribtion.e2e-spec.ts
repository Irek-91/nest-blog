  ...
  describe('create blogger tests', () => {
    ...
    it('Подписываемся к блогу 1 пользователем 2', async () => {
      ...
      const res1 = await request(httpServer)
        .post(`/blogs/${123}/subscription`)
        .set(userTwo.headers);
      expect(res1.status).toBe(404);

      const res2 = await request(httpServer).post(
        `/blogs/${blogOne.id}/subscription`,
      );
      expect(res2.status).toBe(401);

      const res3 = await request(httpServer)
        .post(`/blogs/${blogOne.id}/subscription`)
        .set(userTwo.headers);
    ...
    it('Отписываемя от блога 1, пользователем 2', async () => {
      ...
      const res1 = await request(httpServer)
        .delete(`/blogs/${123}/subscription`)
        .set(userTwo.headers);
      expect(res1.status).toBe(404);

      const res2 = await request(httpServer).delete(
        `/blogs/${blogOne.id}/subscription`,
      );
      expect(res2.status).toBe(401);

      const res = await request(httpServer)
        .delete(`/blogs/${blogOne.id}/subscription`)
        .set(userTwo.headers);
