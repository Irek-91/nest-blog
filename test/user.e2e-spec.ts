...
describe('tests for users', () => {
  ...
  describe('create user in the system ', () => {
    ...
    it('should return 400 status code with errors', async () => {
      ...
      const firstRes = await createUser(
        'admin',
        'qwerty',
        modelOne,
        httpServer,
      ...
      const freeRes = await createUser(
        'admin',
        'qwerty',
        modelFree,
        httpServer,
    ...
    it('фильтрация и пагинация по email и login', async () => {
      ...
      const res = await request(httpServer)
        .get(
          `/sa/users?pageSize=15&pageNumber=1&searchLoginTerm=Th&searchEmailTerm=ee&sortDirection=asc&sortBy=login`,
        )
        .set({ Authorization: 'Basic YWRtaW46cXdlcnR5' });
      expect(res.status).toBe(200);
      expect(res.body).toEqual({
    ...
    it('запрещенный пользователь не должен входить в систему', async () => {
      ...
      const res3 = await request(httpServer).post(`/auth/login`).send({
    ...
    it('фильтрация и пагинация поиск незабаненных пользователей', async () => {
      ...
      const res = await request(httpServer)
        .get(
          `/sa/users?pageSize=15&pageNumber=1&sortDirection=asc&banStatus=notBanned`,
        )
        .set({ Authorization: 'Basic YWRtaW46cXdlcnR5' });
      expect(res.status).toBe(200);
      expect(res.body).toEqual({
    ...
    it('фильтрация и пагинация поиск всех (незабаненных и забаненных) пользователей', async () => {
      ...
      const res = await request(httpServer)
        .get(
          `/sa/users?pageSize=15&pageNumber=1&sortDirection=asc&banStatus=all`,
        )
        .set({ Authorization: 'Basic YWRtaW46cXdlcnR5' });
      expect(res.status).toBe(200);
      expect(res.body).toEqual({
    ...
    it('разбанить третьего пользователя и проверка возможности авторизации', async () => {
      ...
      const res4 = await request(httpServer).post(`/auth/login`).send({
        loginOrEmail: modelThree.login,
        password: modelThree.password,
      });
