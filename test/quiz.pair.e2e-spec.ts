...
describe('tests for questions', () => {
  ...
  describe('Создаем вопросы для тестирования', () => {
    it('Создали 5 вопросов ', async () => {
      await createFiveQuestions('admin', 'qwerty', httpServer);
    });
  });
  ...
  describe('Подключить текущего пользователя к существующей случайной паре или создайте новую пару - connection,', () => {
    ...
    it(`Возвращает статус 201, после ответов первого(userThree) и  второго (userFour)
      ...
      const answeruserThree1 = await sendAnswerOneByUser(
        userThree,
        inputData,
        httpServer,
      );
      const answeruserFour1 = await sendAnswerOneByUser(
        userFour,
        inputData,
        httpServer,
      );

      const answeruserThree2 = await sendAnswerOneByUser(
        userThree,
        inputIncorrectData,
        httpServer,
      );
      const answeruserFour2 = await sendAnswerOneByUser(
        userFour,
        inputIncorrectData,
        httpServer,
      );

      const answeruserThree3 = await sendAnswerOneByUser(
        userThree,
        inputData,
        httpServer,
      );
      const answeruserFour3 = await sendAnswerOneByUser(
        userFour,
        inputIncorrectData,
        httpServer,
      );

      const answeruserThree4 = await sendAnswerOneByUser(
        userThree,
        inputData,
        httpServer,
      );
      const answeruserFour4 = await sendAnswerOneByUser(
        userFour,
        inputIncorrectData,
        httpServer,
      );

      const answeruserThree5 = await sendAnswerOneByUser(
        userFour,
        inputIncorrectData,
        httpServer,
      );
      const answeruserFour5 = await sendAnswerOneByUser(
        userThree,
        inputIncorrectData,
        httpServer,
      );
    ...
    it(`Игра первого(userThree) игрока с третьим(userFive) игроком, результат ничья, 
      ...
      const resultAnswer6 = await sendAnswerOneByUser(
        userThree,
        inputData,
        httpServer,
      );
      ...
      const resultAnswer7 = await sendAnswerOneByUser(
        userThree,
        inputData,
        httpServer,
      );
      ...
      const resultAnswer8 = await sendAnswerOneByUser(
        userThree,
        inputData,
        httpServer,
      );
      ...
      const resultAnswer9 = await sendAnswerOneByUser(
        userThree,
        inputData,
        httpServer,
      );
      ...
      const resultAnswer10 = await sendAnswerOneByUser(
        userThree,
        inputData,
        httpServer,
      );
  ...
  describe('Возвращаем статистику TOP /pair-game-quiz/users/top', () => {
    it('Возвращает статус 200', async () => {
      ...
      const creatResponse = await request(httpServer)
        .get(
          '/pair-game-quiz/users/top?pageSize=3&pageNumber=1&sort=avgScores desc',
        )
        .expect(200);
      expect(creatResponse.body).toEqual({
        pagesCount: 1,
  ...
  describe('тестируем окончание игры, после 10 секунд', () => {
    it('Создаем новуб пару с userThree', async () => {
      ...
      const AccessTokenUserThree = jwt.sign(
        { userId: userThree.id },
        process.env.JWT_SECRET!,
        { expiresIn: 100 },
      );
      const headersJWTuserThree = {
        Authorization: `Bearer ${AccessTokenUserThree}`,
      };
    ...
    it('Подключаемся к userThree игроком userFive', async () => {
      ...
      const AccessTokenUserFive = jwt.sign(
        { userId: userFive.id },
        process.env.JWT_SECRET!,
        { expiresIn: 100 },
      );
      const headersJWTuserFive = {
        Authorization: `Bearer ${AccessTokenUserFive}`,
      };
