...
} from '../src/posts/model/post-model';
...
describe('tests for posts', () => {
  ...
  describe('return post tests', () => {
    it('return posts ', async () => {
      const creatResponse = await request(httpServer).get('/posts').expect(200);
    ...
    it('error 404 is returned, there is no such user', async () => {
      await request(httpServer).get('/posts/:5').expect(404);
    });
  });
  ...
  describe('create post tests', () => {
    it('создаем пост', async () => {
      ...
      const creatResponse = await createPost(
        'admin',
        'qwerty',
        data,
        httpServer,
    ...
    it('создаем пост специальный', async () => {
      ...
      const creatResponse = await createPostSpecific(
        'admin',
        'qwerty',
        blog.id,
        data,
    ...
    it('создание коментария для поста', async () => {
      ...
      const createCommment = await createComment(
        post.id,
        newCommentData,
        201,
        userOne.user.id,
      ...
      const commentIncorect = await createComment(
        post.id,
        dataIncorect,
        400,
        userOne.user.id,
    ...
    it('лайк поста не корректным body likeStatus', async () => {
      ...
      const getResultUpdateLikeIncorect = await request(httpServer)
    ...
    it('лайк поста одним пользователем', async () => {
      ...
      const updateLike = await request(httpServer)
        .put(`/posts/${post.id}/like-status`)
        .set(userOne.headers)
        .send(like)
        .expect(204);

      const resUpdateLikeForUserOne = await request(httpServer)
        .get(`/posts/${post.id}`)
        .set(userOne.headers);
    ...
    it('лайк поста повторно одним пользователем like of like, ', async () => {
      ...
      const againUpdateLike = await request(httpServer)
        .put(`/posts/${post.id}/like-status`)
        .set(userOne.headers)
        .send(like)
        .expect(204);

      const resUpdateLikeOfLike = await request(httpServer)
        .get(`/posts/${post.id}`)
        .set(userOne.headers);
      ...
      const updateLikeOfDislike = await request(httpServer)
        .put(`/posts/${post.id}/like-status`)
        .set(userOne.headers)
        .send(dislike)
        .expect(204);
      const resUpdateLikeOfDislike = await request(httpServer)
        .get(`/posts/${post.id}`)
        .set(userOne.headers);
  ...
  it('лайк поста', async () => {
    ...
    const userResponseThree = await createUser(
      'admin',
      'qwerty',
      userModelThree,
      httpServer,
    ...
    const userResponseFour = await createUser(
      'admin',
      'qwerty',
      userModelFour,
      httpServer,
    ...
    const userResponseFive = await createUser(
      'admin',
      'qwerty',
      userModelFive,
      httpServer,
    ...
    const getResultUpdateLike = await request(httpServer)
      .put(`/posts/${post.id}/like-status`)
      .set(userOne.headers)
      .send(like)
      .expect(204);
    const getResultUpdateLikeTwo = await request(httpServer)
      .put(`/posts/${post.id}/like-status`)
      .set(userTwo.headers)
      .send(dislike)
      .expect(204);
    const getResultUpdateFree = await request(httpServer)
      .put(`/posts/${post.id}/like-status`)
      .set(userThree.headers)
      .send(like)
      .expect(204);
    const getResultUpdateFour = await request(httpServer)
      .put(`/posts/${post.id}/like-status`)
      .set(userFour.headers)
      .send(like)
      .expect(204);
    const getResultUpdateFive = await request(httpServer)
      .put(`/posts/${post.id}/like-status`)
