...
export class AddTelegramIdBySubscriberUseCase
  implements ICommandHandler<AddTelegramIdBySubscriberCommand>
{
  constructor(private blogsRepository: BlogsRepoPSQL) {}
  async execute(
    command: AddTelegramIdBySubscriberCommand,
  ): Promise<true | null> {
    const code = command.code;
    const telegramId = command.telegramId;
    return await this.blogsRepository.addTelegramIdForSuscriber(
      code,
      telegramId,
    );
  }
