...
export class UpdateBanStatusByBlogUseCase implements ICommandHandler<UpdateBanStatusByBlogCommand> {
    constructor (private blogsRepository: BlogsRepoPSQL) {

    }
    async execute(command: UpdateBanStatusByBlogCommand): Promise<boolean | null> {
        const blogId = command.blogId
        const banStatus = command.banStatus
        return await this.blogsRepository.updateBanStatusByBlogId(blogId, banStatus)
    }
