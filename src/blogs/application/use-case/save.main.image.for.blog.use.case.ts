...
export class SaveMainImageForBlogUseCase {
    ...
    async execute(command: SaveMainImageForBlogCommand): Promise<blogsImageWiewModel | null> {
        const userId = command.userId
        const file = command.file
        const blogId = command.blogId
        const result = await this.s3StorageAdapter.saveMainImageForBlog(userId, file)
        if (!result) {
            return null
        }
        const saveInfoInDB = await this.blogsRepository.saveInfoByMainImageInDB(blogId, result.url, result.fileId, file.size)
        if (!saveInfoInDB) {
            return null
        }
