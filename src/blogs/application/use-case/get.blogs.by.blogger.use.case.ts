...
export class GetBlogsByBloggerUseCase implements ICommandHandler<GetBlogsByBloggerCommand>  {
    ...
    async execute(command: GetBlogsByBloggerCommand): Promise<Blog | null> {
        const blogs = await this.blogsQueryRepository.getBlogsByBlogger(command.bloggerId)
        return blogs
    }
