...
export class SaveWallpaperImageForBlogUseCase {
  ...
  async execute(
    command: SaveWallpaperImageForBlogCommand,
  ): Promise<blogsImageWiewModel | null> {
    const userId = command.userId;
    const file = command.file;
    const blogId = command.blogId;
    const result = await this.s3StorageAdapter.saveWallpaperImageForBlog(
      userId,
      file,
    );
    if (!result) {
    ...
    const saveInfoInDB =
      await this.blogsRepository.saveInfoByWallpaperImageInDB(
        blogId,
        result.url,
        result.fileId,
        file.size,
