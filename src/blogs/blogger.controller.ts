...
import {
  postInputModelSpecific,
  postImagesViewModel,
} from '../posts/model/post-model';
...
import { paginatorPost, postOutput } from '../posts/model/post-model';
...
export class BloggerController {
  ...
  async getBlogs(
    ...
    const blogs: paginatorBlog = await this.commandBus.execute(
      new FindBlogsCommand(queryFilter, userId),
    );
    if (!blogs) {
  ...
  async getCommentsPostsByBlog(
    ...
    const queryFilter = this.pagination.getPaginationFromQuery(query);
    const comments: paginationGetCommentsByBlog | null =
      await this.commandBus.execute(
        new GetCommentsByBlogCommand(userId, queryFilter),
      );
  ...
  async getPostsByBlogId(
    ...
    const foundPosts: paginatorPost | null = await this.commandBus.execute(
  ...
  async createBlogByBlogger(
    ...
    const blog = await this.commandBus.execute(
      new CreateBlogCommand(blogInputData, userId),
    );
    return blog;
  }
  ...
  async createPostByBlog(
    ...
    const newPost: postOutput | null = await this.commandBus.execute(
  ...
  async saveMainImageForBlog(
    @UploadedFile(new FileValidationPipe(), new FileMainValidationPipe())
    ...
    const newMainImage: blogsImageWiewModel | null =
      await this.commandBus.execute(
        new SaveMainImageForBlogCommand(blogId, userId, file),
      );

    if (!newMainImage) {
      throw new HttpException(
        'Not Found SaveMainImageForBlogCommand',
  ...
  @UseInterceptors(FileInterceptor('file'))
  async saveWallpaperImageForBlog(
    @UploadedFile(new FileValidationPipe(), new FileWallpaperValidationPipe())
    file: Express.Multer.File,
    @Param('blogId', new PipeisValidUUID()) blogId: string,
    @Request() req: any,
  ) {
    ...
    const newMainImage: blogsImageWiewModel | null =
      await this.commandBus.execute(
        new SaveWallpaperImageForBlogCommand(blogId, userId, file),
      );
  ...
  async saveImagesForPost(
    @UploadedFile(new FileValidationPipe(), new PostImageValidationPipe())
    file: Express.Multer.File,
    @Param('blogId', new PipeisValidUUID()) blogId: string,
    ...
    const newMainImage: postImagesViewModel | null =
      await this.commandBus.execute(
        new SaveImageForPostCommand(blogId, userId, postId, file),
      );

    if (!newMainImage) {
      throw new HttpException(
        'Not Found SaveImageForPostCommand',
        HttpStatus.NOT_FOUND,
      );
    } else {
      return newMainImage;
  ...
  async deletePostByBlogId(
    ...
    const result = await this.commandBus.execute(
      new DeletePostIdCommand(postId),
    );
  ...
  @Delete('blogs/:id')
  async deletBlog(@Param('id') blogId: string, @Request() req: any) {
    const userId = req.userId; //исправить после авторизации
    const blog: Blog | null = await this.commandBus.execute(
      new GetBlogDBCommand(blogId),
    );
