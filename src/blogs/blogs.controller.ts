...
export class BlogsController {
  ...
  async getBlogId(@Param('id') blogId: string, @Request() req: any) {
    ...
    const blog = await this.commandBus.execute(
      new GetBlogIdCommand(blogId, userId),
    );
    if (!blog) {
  ...
  async getPostsByBlogId(
    ...
    const blog: blogOutput | null = await this.commandBus.execute(
  ...
  async subscriptionUserToBlog(
    ...
    const checkStatus: true | null = await this.commandBus.execute(
      new CheckStatusSubscriptionUserCommand(
        blogId,
        userId,
    ...
    if (!checkStatus) {
      const res = await this.commandBus.execute(
        new SubscriptionUserToBlog(blogId, userId),
      );
    }
  }
  ...
  async unsubscribeUserToBlog(
    ...
    if (!checkStatus) {
      const res = await this.commandBus.execute(
        new UnsubscribeUserToBlog(blogId, userId),
      );
    }
  }
}
