...
export class FilterCountIPAndURL implements CanActivate {
  constructor(
    @InjectModel(IPAndURIModel.name)
    private ipAndURIModel: Model<IPAndURIDocument>,
  ) {}
  ...
  async canActivate(context: ExecutionContext): Promise<any> {
    ...
    const count = await this.ipAndURIModel.countDocuments({
      IP: newAPI.IP,
      URL: newAPI.URL,
...
export class CheckRefreshToken {
  ...
  async canActivate(context: ExecutionContext): Promise<any> {
    ...
    const validationToken =
      await this.jwtService.checkingTokenKey(cookiesRefreshToken);
    ...
    const expiredToken =
      await this.securityDeviceService.findTokenAndDevice(cookiesRefreshToken);
    if (expiredToken.result === null)
      throw new HttpException(
        ['UNAUTHORIZED expired', expiredToken],
        HttpStatus.UNAUTHORIZED,
      );
...
@Injectable()
export class CheckRefreshTokenDeleteDevice {
  constructor(
    protected jwtService: JwtService,
    protected securityDeviceService: SecurityDeviceServicePSQL,
  ) {}
...
export class CheckingActivePair implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<any> {
    ...
    const getActivPair = await this.commandBus.execute(
      new GetPairMyCurrentCommand(userId),
    );
    if (!getActivPair) {
