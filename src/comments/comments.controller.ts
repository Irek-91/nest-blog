...
export class CommentsController {
    ...
    async getCommentById(
        ...
        let comment = await this.commandBus.execute(new GetCommentByIdCommand(commentId, userId))
        if (!comment) {
    ...
    async updateCommentId(@Param('commentId') commentId: string,
        ...
        let resultContent = await this.commandBus.execute(new UpdateCommentByPostCommand(userId, commentId, commentInputData.content))
    ...
    @Request() req: any
        ...
        const userId = req.userId
        const result = await this.commandBus.execute(new UpdateLikeStatusCommentCommand(commentId, userId, likeStatus.likeStatus))
        if (result === true) {
            throw new HttpException('No Content', HttpStatus.NO_CONTENT)
        }
        if (result === null) {
    ...
    @Request() req: any
        ...
        const userId = req.userId
        const result = await this.commandBus.execute(new DeleteCommentByIdCommand (commentId, userId))
        if (result === false) {
            throw new HttpException('If try edit the comment that is not your own', HttpStatus.FORBIDDEN)
        }
        if (result === true) {
