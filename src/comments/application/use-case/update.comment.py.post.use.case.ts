...
export class UpdateCommentByPostCommand {
    constructor(public userId: string, public commentsId: string, public content: string) {

    }
}

@CommandHandler(UpdateCommentByPostCommand)
export class UpdateCommentByPostUseCase implements ICommandHandler<UpdateCommentByPostCommand> {
    constructor(private commentsQueryRepository: CommentsQueryRepoPSQL,
        private commentsRepository: CommentsRepoPSQL) {

    }

    async execute(command: UpdateCommentByPostCommand): Promise<true | false | null> {
