...
export class CreatedCommentPostUseCase
  implements ICommandHandler<CreatedCommentPostCommand>
{
  constructor(
    private usersQueryRepository: UsersQueryRepoPSQL,
    private commentsRepository: CommentsRepoPSQL,
  ) {}
  async execute(
    command: CreatedCommentPostCommand,
    ...
    const bannedBlog =
      await this.usersQueryRepository.getUserBannedByBlogger(userId);
    if (!bannedBlog || bannedBlog.blogId._id !== blogId) {
      const creatComment = await this.commentsRepository.createdCommentPostId(
        postId,
        content,
        userId,
        userLogin,
