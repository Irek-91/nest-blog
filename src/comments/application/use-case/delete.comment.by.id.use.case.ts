...
export class DeleteCommentByIdCommand {
    constructor(public commentsId: string, public userId: string) {
    }
}
...
export class DeleteCommentByIdUseCase implements ICommandHandler<DeleteCommentByIdCommand>{
    ...
    async execute(command: DeleteCommentByIdCommand): Promise<boolean| null> {
        ...
        try {
            const commentById = await this.commentsQueryRepository.findCommentById(command.commentsId, command.userId)
            if (!commentById) {
                return null
            }
            if (commentById.commentatorInfo.userId === command.userId) {
                const result = await this.commentsRepository.deletCommentById(command.commentsId)
                if (result === true) {
                    return true
                } else {
                    return null
