...
export class UpdateLikeStatusCommentUseCase implements ICommandHandler<UpdateLikeStatusCommentCommand>{
...
async execute(command: UpdateLikeStatusCommentCommand): Promise<true | null> {
    const result = await this.commentsQueryRepository.findCommentById(command.commentId, command.userId)
    if (!result) {return null}
    return this.commentsRepository.updateLikeStatus(command.commentId, command.userId, command.likeStatus)
}
}
