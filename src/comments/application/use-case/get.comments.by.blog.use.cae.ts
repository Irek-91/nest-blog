...
import { paginationGetCommentsByBlog } from '../../model/comments-model';
...
export class GetCommentsByBlogCommand {
  constructor(
    ...
    public pagination: queryPaginationType,
  ) {}
}
  ...
  implements ICommandHandler<GetCommentsByBlogCommand>
{
  ...
  async execute(
    command: GetCommentsByBlogCommand,
  ): Promise<paginationGetCommentsByBlog | null> {
    const blog = await this.blogsQueryRepository.getBlogsByBlogger(
      command.userId,
    );
    if (!blog) {
      return null;
    }
    const comments = await this.commentsQueryRepository.findCommentsByBlog(
      blog._id,
      command.userId,
      command.pagination,
    );
