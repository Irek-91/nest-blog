...
export class S3StorageAdapter {
  ...
  constructor() {
    ...
    this.s3client = new S3Client({
      ...
      endpoint: ENDPOINT,
      credentials: {
        secretAccessKey: process.env.S3_ADAPTER_KEY!,
        accessKeyId: process.env.S3_ADAPTER_KEY_ID!,
      },
    });
  }
  ...
  ): Promise<{ url: string; fileId: any } | null> {
  ...
  async saveImageForPost(
    ...
    try {
      const uploadOroiginal: PutObjectCommandOutput =
        await this.s3client.send(commandOriginal);
      const uploadMiddle: PutObjectCommandOutput =
        await this.s3client.send(commandMiddle);
      const uploadSmall: PutObjectCommandOutput =
        await this.s3client.send(commandSmall);
