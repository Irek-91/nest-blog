...
export class UpdateStatusUserUseCase
  implements ICommandHandler<UpdateStatusUserCommand>
{
  constructor(
    private usersRepo: UsersRepositoryPSQL,
  ...
  async execute(command: UpdateStatusUserCommand): Promise<User | null> {
    ...
    const resultUser = await this.usersRepo.updateStatusUser(
      command.userId,
      command.isBanned,
      banReason,
      banDate,
    ...
    if (command.isBanned) {
      await this.securityDeviceRepoP.deleteAllDevicesByUserId(command.userId);
    }
    if (!resultUser) {
      return null;
    }
