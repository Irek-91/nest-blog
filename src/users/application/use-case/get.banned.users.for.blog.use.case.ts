...
export class GetBannedUsersForBlogCommand {
    constructor(public blogId: string, public paginationFilter: queryPaginationTypeUserSA) {
    }
}
...
export class GetBannedUsersForBlogUseCase implements ICommandHandler<GetBannedUsersForBlogCommand> {
    ...
    async execute(command: GetBannedUsersForBlogCommand): Promise<bannedUsersViewModel> {
        const blogId = command.blogId
        const paginatorUser = command.paginationFilter
        const bannedUsers = await this.userRepo.getUsersBannedByBlogId(blogId, paginatorUser)
        return bannedUsers
