...
export class SecurityDeviceRepoPSQL {
  ...
  async deleteDevicesAll() {
    const deletResult = await this.dataSource
      .createQueryBuilder()
      .delete()
      .from(Device)
      .execute();
  ...
  async deleteAllDevicesExceptOne(
    ...
    try {
      const checkUserIdByDeviceId = await this.dataSource
        .getRepository(Device)
        .createQueryBuilder()
        .select()
        .where({ deviceId: deviceId })
