...
export class UpdateQuestionIdUseCase
  implements ICommandHandler<UpdateQuestionIdComand>
{
  constructor(
  ...
  async execute(command: UpdateQuestionIdComand): Promise<boolean> {
    ...
    const updateResult = await this.questionsRepository.updateQuestionById(
      command.inputModel,
      command.questionId,
    );
    return updateResult;
