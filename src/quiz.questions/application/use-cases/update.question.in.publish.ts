...
export class UpdateQuestionInPublishUseCase
  implements ICommandHandler<UpdateQuestionInPublishCommand>
{
  constructor(
  ...
  async execute(command: UpdateQuestionInPublishCommand): Promise<boolean> {
    const question = await this.questionsQueryRepository.getQuestiontById(
      command.questionId,
    );
    if (!question) {
      return false;
    }
    const updateResult = await this.questionsRepository.updateQuestionInPublish(
      command.inputModel,
      command.questionId,
    );
    return updateResult;
