...
export class PairGameController {
  ...
  async getPairMyCurrent(@Request() req: any) {
    ...
    const pairMyCurrent: gamePairViewModel | null =
      await this.commandBus.execute(new GetPairMyCurrentCommand(userId));
  ...
  async getStatisticByUser(@Request() req: any) {
    ...
    const myStatistic: myStatisticViewModel = await this.commandBus.execute(
      new GetStatisticByUserCommand(userId),
    );
    return myStatistic;
  }
  ...
  async getUsersTop(
    ...
    const queryFilter: queryPaginationTopUsersType =
      this.pagination.getPaginationFromQueryTopUsers(query);

    const getUsersTop: topGamePlayerViewModel = await this.commandBus.execute(
      new GetTopUsersCommand(queryFilter),
    );
  ...
  async getAllMyPairs(
    ...
    const userId = req.userId;
    const queryFilter: queryPaginationPairsType =
      this.pagination.getPaginationFromQueryPairs(query);

    const pairsAllbyUser: gameAllPairsViewModel | null =
      await this.commandBus.execute(
        new GetAllPairsByUserCommand(queryFilter, userId),
      );
  ...
  @Get('/pairs/:id')
  async getPairById(
    @Param('id', new PipeisValidUUID()) pairId: string,
    @Request() req: any,
  ) {
    const userId = req.userId;

    const pairMyCurrent: gamePairViewModel | null =
      await this.commandBus.execute(new GetPairByIdCommand(pairId, userId));
  ...
  @HttpCode(200)
  async connectUserByPair(@Request() req: any) {
    const userId = req.userId;
    const createdNew = await this.commandBus.execute(
      new CreateNewStatisticByPalyerCommand(userId),
    );
    const pair = await this.commandBus.execute(
      new ConnectUserByPairCommand(userId),
    );
    if (!pair) {
      throw new HttpException('Not Found', HttpStatus.NOT_FOUND);
    }
  ...
  @HttpCode(200)
  async sendAnswer(@Body() inputDate: AnswerInputModel, @Request() req: any) {
    const userId = req.userId;
    const result = await this.commandBus.execute(
      new SendAnswerCommand(inputDate, userId),
    );

    if (result === 403) {
      throw new HttpException(
        `If current user is not inside active pair or u
            ser is in active pair but has already answered to all questions`,
