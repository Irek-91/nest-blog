    ...
    async getTopUsers(queryFilter: queryPaginationTopUsersType): Promise<userStatisticViewModel[] | []> {
        try {
            ...
            let result = await this.dataSource
                ...
                .select('*')
                .addSelect(`CAST((s.sumScore / (s.winsCount + s.lossesCount + s.drawcount)) AS decimal(18,2)) AS "avgScores"`)
            ...
            const result1 = queryFilter.sort.forEach(async function callback(element, index) {
            ...
            const resultOne: userStatisticViewModel[] = await Promise.all(res.map(async (b) => {
