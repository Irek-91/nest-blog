...
import { answerViewModel } from '../model/games.model';
...
export class PairGameRepo {
  ...
  async addPlayerInPair(
    ...
    try {
      const addPlayerPair = await manager.update(
        Pair,
        {
          id: pairId,
        },
      ...
      const createPesultPlayer = await manager.insert(Pairresult, {
        id: uuidv4(),
        pairId: pairId,
  ...
  async createNewStatistic(userId: string): Promise<boolean> {
    ...
    await queryRunner.startTransaction();
    try {
      const createPesultPlayer = await manager.insert(Statistic, {
        userId: { _id: userId },
        sumScore: 0,
        winsCount: 0,
        lossesCount: 0,
  ...
  async createNewPair(playerId: string): Promise<string | null> {
    ...
    try {
      ...
      const createNewPair = await manager.insert(Pair, {
        id: pairId,
  ...
  async updateResultAnswer(
    ...
    try {
      ...
      const resultPlayer =
        await this.pairGameQueryRepo.getResultPairsByPlayerId(pairId, playerId);
      ...
      const finishGameDate: string | null = null;
  ...
  async updateStatusByPair(pairId: string): Promise<boolean | null> {
    ...
    try {
      const updateStatus = await manager.update(
        Pair,
        {
          id: pairId,
        },
  ...
  async updateStatusGame(
    ...
    try {
      const updatePairResultLoserPlayer = await manager.update(
        Pairresult,
        {
          pairId: pairId,
      ...
      const updatePairResultWinnerPlayer = await manager.update(
        Pairresult,
        {
          pairId: pairId,
      ...
      const addAndUpdateStatisticWinner = await manager.update(
        Statistic,
        {
      ...
      const addAndUpdateStatisticLoser = await manager.update(
        Statistic,
        {
  ...
  async resultUpdateIsAdraw(
    ...
    try {
      ...
      const updatePairResultPlayerTwo = await manager.update(
        Pairresult,
        {
          pairId: pairId,
      ...
      const updatePairResultPlayerOne = await manager.update(
        Pairresult,
        {
          pairId: pairId,
      ...
      const addAndUpdateStatisticPlayerOne = await manager
        .createQueryBuilder()
        .update(Statistic)
        .set({
      ...
      const addAndUpdateStatisticPlayerTwo = await manager
        .createQueryBuilder()
        .update(Statistic)
        .set({
  ...
  async deleteAll() {
    ...
    await queryRunner.startTransaction();
    try {
      const resultTwo = queryRunner.manager.delete(Pairresult, {});

      const resultThree = await queryRunner.manager.delete(Statistic, {});

      const result = await queryRunner.manager.delete(Pair, {});
