...
export class GetStatisticByUserUseCase
  implements ICommandHandler<GetStatisticByUserCommand>
{
  constructor(private pairGameQueryRepo: PairGameQueryRepo) {}

  async execute(
    command: GetStatisticByUserCommand,
  ): Promise<myStatisticViewModel> {
    const result = await this.pairGameQueryRepo.getStatisticByUser(
      command.userId,
    );
    if (!result) {
      return {
    ...
    const gamesCount =
      +result.winsCount + +result.lossesCount + +result.drawcount;
