...
export class SendAnswerCommand {
  constructor(
    public inputDate: AnswerInputModel,
...
export class SendAnswerUseCase implements ICommandHandler<SendAnswerCommand> {
  ...
  async execute(
    command: SendAnswerCommand,
  ): Promise<answerViewModel | null | 403> {
    ...
    const resultPlayer = await this.pairGameQueryRepo.getResultPairsByPlayerId(
      pair.id,
      playerId,
    );
    ...
    const numberQuestion = resultPlayer!.answersAddedAt.length;
    const questionId = pair.questionsId[numberQuestion];

    const resultAnswer = await this.qusetionsService.checkingCorrectAnswer(
      questionId,
      answer,
    ); //проверяем ответ, если правильно то true
    ...
    const updateResultAnswer = await this.pairGameRepo.updateResultAnswer(
      pair.id,
      questionId,
      playerId,
      statusAnswer,
    ...
    const resultUpdateFirstPlayer =
      await this.pairGameQueryRepo.getResultPairsByPlayerId(
        pair.id,
        pair.firstPlayerId,
      );
    const resultUpdateSecondPlayer =
      await this.pairGameQueryRepo.getResultPairsByPlayerId(
        pair.id,
        pair.secondPlayerId,
      );
    ...
    if (
      resultUpdateFirstPlayer!.answersAddedAt.length === 5 ||
      resultUpdateSecondPlayer!.answersAddedAt.length === 5
    ) {
      setTimeout(async () => {
        const resPair = await this.pairGameQueryRepo.getPairById(pair.id);
        const resultFirstPlayer =
          await this.pairGameQueryRepo.getResultPairsByPlayerId(
            resPair!.id,
            pair!.firstPlayerId,
          );
        ...
        while (resultSecondPlayer!.answersStatus.length < 5) {
          ...
          resultSecondPlayer!.answersAddedAt.push(
            `${new Date().toISOString()}`,
          );
        }

        const updateResultFirstPlayer =
          await this.pairGameRepo.updateResultFinish(
            resPair!.id,
            pair!.firstPlayerId,
            resultFirstPlayer!.answersStatus,
            resultFirstPlayer!.answersAddedAt,
          );
        const updateResultSecondPlayer =
          await this.pairGameRepo.updateResultFinish(
            resPair!.id,
            pair!.secondPlayerId,
            resultSecondPlayer!.answersStatus,
            resultSecondPlayer!.answersAddedAt,
          );
        const updateStatusByPair = await this.pairGameRepo.updateStatusByPair(
          resPair!.id,
        );

        const resultFinishFirstPlayer =
          await this.pairGameQueryRepo.getResultPairsByPlayerId(
            resPair!.id,
            pair!.firstPlayerId,
          );
        const resultFinishSecondPlayer =
          await this.pairGameQueryRepo.getResultPairsByPlayerId(
            resPair!.id,
            pair!.secondPlayerId,
          );
        ...
        scoreOne = resultFinishFirstPlayer!.answersStatus.reduce(function (
          accumulator,
          item,
        ) {
          if (item === 'Correct') {
        ...
        scoreTwo = resultFinishSecondPlayer!.answersStatus.reduce(function (
          accumulator,
          item,
        ) {
          if (item === 'Correct') {
        ...
        if (scoreTwo !== scoreOne) {
          const updateDateFinish = await this.pairGameRepo.updateStatusGame(
            resPair!.id,
            winnerPlayer,
            loserPlayer,
          );
        ...
        if (scoreTwo === scoreOne) {
          const updateDateFinish = await this.pairGameRepo.resultUpdateIsAdraw(
            resPair!.id,
            winnerPlayer,
            loserPlayer,
          );
