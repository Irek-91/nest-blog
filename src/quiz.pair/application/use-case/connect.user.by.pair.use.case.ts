...
export class ConnectUserByPairUseCase implements ICommandHandler<ConnectUserByPairCommand>{
    ...
    async execute(command: ConnectUserByPairCommand): Promise<gamePairViewModel | null> {
        ...
        if (!secondPlayerPair) {
            ...
            const result = await this.pairGameService.getPairMuCurrentViewModel(userId, newPairId!)
            return result
        } else {
            const addPlayerInPairId = await this.pairGameRepo.addPlayerInPair(userId, secondPlayerPair.id)
            const result = await this.pairGameService.getPairMuCurrentViewModel(userId, addPlayerInPairId!)
            return result
        }
    }
}