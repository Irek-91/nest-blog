...
export class GetPairByIdCommand {
    constructor(public pairId: string, public userId: string) {
    }
}
...
async execute(command: GetPairByIdCommand): Promise<gamePairViewModel | null> {
        ...
        const result = await this.pairGameService.getPairMuCurrentViewModel(userId, pairId)
