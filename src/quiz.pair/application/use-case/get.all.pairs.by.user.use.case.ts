...
export class GetAllPairsByUserCommand {
  constructor(
    public queryFilter: queryPaginationPairsType,
    public userId: string,
  ) {}
}
@CommandHandler(GetAllPairsByUserCommand)
export class GetAllPairsByUserUseCase
  implements ICommandHandler<GetAllPairsByUserCommand>
{
  constructor(
  ...
  async execute(
    command: GetAllPairsByUserCommand,
  ): Promise<gameAllPairsViewModel | null> {
    const userId = command.userId;
    const queryFilter = command.queryFilter;
    const pairs = await this.pairGameQueryRepo.getAllPairsByUser(
      userId,
      queryFilter,
    );
    if (!pairs) {
    ...
    let result: any = await Promise.all(
      pairs.map(async (p) => {
        return await this.pairGameService.getPairMuCurrentViewModel(
          userId,
          p.id,
        );
      }),
