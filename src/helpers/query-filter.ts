...
export class Pagination {
  ...
  getPaginationFromQuery = (query: any): queryPaginationType => {
    ...
    if (query.sortBy === 'createdAt' || query.sortBy === 'name') {
      defaultValues.sortBy = query.sortBy;
    } else {
      query.sortBy = 'createdAt';
    }
    if (query.sortDirection) {
      defaultValues.sortDirection = query.sortDirection.toUpperCase();
    }
    if (query.pageNumber) {
    ...
    defaultValues.skip =
      (defaultValues.pageNumber - 1) * defaultValues.pageSize;
  ...
  getPaginationFromQueryQuestions = (
    query: any,
  ): queryPaginationQuestionsType => {
    ...
    if (query.bodySearchTerm) {
      defaultValues.bodySearchTerm = query.bodySearchTerm;
    }
    ...
    if (
      ...
      defaultValues.publishedStatus = query.publishedStatus;
    } else {
      defaultValues.publishedStatus === 'all';
    }
    ...
    if (
      ...
      defaultValues.sortDirection = 'DESC';
    }
  ...
  getPaginationFromQueryPairs = (query: any): queryPaginationPairsType => {
    ...
    if (
      query.sortBy === 'status' ||
      query.sortBy === 'startGameDate' ||
      query.sortBy === 'finishGameDate'
    ) {
      defaultValues.sortBy = query.sortBy;
    } else {
      defaultValues.sortBy = 'pairCreatedDate';
    }
  ...
  getPaginationFromQueryTopUsers = (
    query: any,
    ...
    if (typeof resultFilter == 'object') {
      const array = resultFilter.join(' ').split(' ');
      ...
      const array = resultFilter.split(' ');
      resultFilter = array.reduce(function (result, value, index) {
