...
export const chekAnswer = async (
  pairId: string,
  playerId: string,
  pairGameRepo: PairGameRepo,
  pairGameQueryRepo: PairGameQueryRepo,
) => {
  const pair = await pairGameQueryRepo.getPairById(pairId);
  const resultFirstPlayer = await pairGameQueryRepo.getResultPairsByPlayerId(
    pairId,
    pair!.firstPlayerId,
  );
  const resultSecondPlayer = await pairGameQueryRepo.getResultPairsByPlayerId(
    pairId,
    pair!.secondPlayerId,
  );
  ...
  await pairGameRepo.updateResultFinish(
    pairId,
    pair!.firstPlayerId,
  ...
  await pairGameRepo.updateStatusByPair(pairId);
  const resultFinishFirstPlayer =
    await pairGameQueryRepo.getResultPairsByPlayerId(
      pairId,
      pair!.firstPlayerId,
    );
  ...
  if (scoreTwo !== scoreOne) {
    await pairGameRepo.updateStatusGame(pair!.id, winnerPlayer, loserPlayer);
  }
  if (scoreTwo === scoreOne) {
    await pairGameRepo.resultUpdateIsAdraw(pair!.id, winnerPlayer, loserPlayer);
  }
  return true;
};
